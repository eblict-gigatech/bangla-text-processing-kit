...
def get_model_object():
    ...
    # Try to allowlist the custom Vocabulary class (adjust import path if needed).
    # WARNING: loading with weights_only=False will execute pickled code. Only do this if you trust the checkpoint source.
    try:
        # adjust the import path if your project places Vocabulary elsewhere
        from dataloaders.vocabulary import Vocabulary
        from torch.serialization import safe_globals

        with safe_globals([Vocabulary]):
            model_object = torch.load(
                model_path, map_location=device, weights_only=False
            )
    except Exception:
        # Fallback: directly load (may still succeed); will unpickle arbitrary objects.
        model_object = torch.load(model_path, map_location=device, weights_only=False)
